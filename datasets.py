...
def _get_tokenizer() -> T5Tokenizer:
    """Lazily create one tokenizer per worker process."""
...
class Base_Dataset(Dataset.Dataset):
    def collate_fn(self, batch):
        ...
        for key in keys:
            ...
            padded_video = [torch.cat((vid[key], vid[key][-1][None].expand(max_len - len(vid[key]), -1, -1)), dim=0) for vid in pose_tmp]
            img_batch = torch.stack(padded_video, 0)
            src_input[key] = img_batch  # (B, T, V, 3)

            if "attention_mask" not in src_input:
                src_length_batch = video_length
                mask_gen = [torch.ones(i, dtype=torch.long) + 7 for i in src_length_batch]
                mask_gen = pad_sequence(mask_gen, padding_value=0, batch_first=True)
                src_input["attention_mask"] = (mask_gen != 0).long()
                src_input["name_batch"] = name_batch
                src_input["src_length_batch"] = src_length_batch

        # -------------- NEW: tokenizer lives here ------------------
        tok = _get_tokenizer()
        B = len(tgt_batch)
        lang_prompt = "Chinese" if getattr(self, "lang", "Chinese") == "Chinese" else "English"
        prompt_str = f"Translate sign language video to {lang_prompt}: "
        prefix_ids = tok([prompt_str] * B, padding="longest", return_tensors="pt").input_ids  # (B, P)
        prefix_mask = (prefix_ids != tok.pad_token_id).long()

        label_ids = tok(tgt_batch, padding=True, truncation=True, max_length=50, return_tensors="pt").input_ids  # (B, L)
        # leave pad tokens; model will set -100 later

        src_input["prefix_ids"] = prefix_ids
        src_input["prefix_mask"] = prefix_mask

        tgt_input = {
            "labels_ids": label_ids,
            "gt_sentence": tgt_batch,            # keep for BLEU/WER routines
